...
# -------------------------
alias tosh="sudo chsh -s /bin/sh && echo 'Now log out.'"
#alias tobash="sudo chsh $USER -s /bin/bash && echo 'Now log out.'"
alias tofish="sudo chsh -s /usr/local/bin/fish && echo 'Now log out.'" # BSD
#alias tofish="sudo chsh $USER -s /usr/bin/fish && echo 'Now log out.'" # Linux
...
# -------------------------
# pkg
alias u='sudo freebsd-update fetch  && sudo freebsd-update install' # jeśli była instalacja kernela to reboot a potem upkg
alias up='sudo pkg update && sudo pkg upgrade'
alias i='sudo pkg install'
alias r='sudo pkg delete'
alias c='sudo pkg autoremove && sudo pkg clean -a '
alias s='pkg search'
alias q='pkg query %n'
alias qg='pkg query %n | grep'
#alias oi='pkg query "%t %n" | sort -n | tail -20 | awk '{cmd="date -r "$1" +\"%Y-%m-%d %H:%M\""; cmd | getline d; close(cmd); print d, $2}''
#alias oi='pkg query "%t %n" | sort -n | awk '{cmd="date -r "$1" +\"%Y-%m-%d %H:%M\""; cmd | getline d; close(cmd); print d, $2}' | grep'
#alias oiapt="awk '/Start-Date:/ {date=\$2\" \"\$3} /Install:/ {print date, \$2}' /var/log/apt/history.log"
